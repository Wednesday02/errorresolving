...
const Edit = () => {
 const users={
  ...
  useEffect(()=>{
   axios.get(`https://abc-p46c.onrender.com/api/getOne/${id}`)
   .then((response)=>{
  ...
  const submitForm=async(e)=>{
     e.preventDefault();
       await axios.put(`https://abc-p46c.onrender.com/api/update/${id}`,user)
       .then((response)=>
       {
  ...
  return (
     <div className='addUser'>
        ...
        <form className='adduserform' onSubmit={submitForm}>
            ...
            <input type='email' value={user.email} onChange={inputChangeHandler} name='email' id='email' autoComplete='off' placeholder='Email Address'/>
            </div>
